...
import { partymap, CheckExternalMerchant } from "../../config";
import { useLedger, useParty, useStreamQueries } from "@daml/react";
import { Table, TableBody, TableCell, TableRow, TableHead, Grid, Paper, Typography, Button} from "@material-ui/core";
import { Offer } from "@daml.js/da-marketplace/lib/SmartCash/ExternalMerchant/Service";
import {OnboardingDialogView} from "./OnboardExternalDialog"
import { VariantType, useSnackbar } from 'notistack';
import { CreateEvent } from "@daml/ledger";
import { AssetSettlementRule } from "@daml.js/da-marketplace/lib/DA/Finance/Asset/Settlement";


export const ExternalServiceRequestView = () => {
  ...
  const assetSettlmentRule = assetSettlementRules.find(i => i.payload.account.id.label == party+"@CashAccount");
              ...
              {offer.map((c, i) => (
                <TableRow key={i} className={classes.tableRow}>
                  ...
                  <TableCell key={2} className={classes.tableCellMini}>
                    { CheckExternalMerchant(party) && <Button color="primary" size="small" className={classes.smallButton} variant="contained" 
